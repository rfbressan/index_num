...
```{r tab-pib, cache=TRUE, include=FALSE}
pib <- read_csv2("../Data/tabela1620.csv", 
                  col_names = c("trimestre", "agro", "industria", 
                                "servicos", "PIB"),
                  skip = 5, n_max = 94)
head(pib)
tail(pib)
...
```{r trimestre, include=FALSE}
pib_tri_ano <- pib %>%
  separate(trimestre, into = c("tri", "ano"), sep = "mestre ") %>% 
  mutate(ano = as.numeric(ano))
```
...
```{r var-tri, include=FALSE}
pib_tri_ano <- pib_tri_ano %>% 
  pivot_longer(-c(tri, ano)) %>% 
  group_by(name) %>% 
  mutate(var_tri = 100*((value - lag(value, 1))/lag(value, 1)))
head(pib_tri_ano)
```
...
```{r suma, include=FALSE}
suma <- pib_tri_ano %>% 
  filter(var_tri == max(var_tri, na.rm = TRUE))
suma
```

2. Calcule a variação percentual acumulada de 12 meses para cada índice e a variação do ano.

```{r var_12m, include=FALSE}
pib_tri_ano <- pib_tri_ano %>% 
  mutate(var_12m = 100*((value - lag(value, 4))/lag(value, 4))) %>% 
  group_by(name, ano) %>% 
  mutate(var_ano = 100*(cumprod(1 + var_tri / 100) - 1))
```

2.1 Plote a variação percentual trimestral do PIB na forma de série de barras, utilizando o ggplot.

```{r plot-barras, include=FALSE}
pib_tri <- pib_tri_ano %>% 
  ungroup() %>% 
  filter(name == "PIB") %>% 
  select(-c(var_12m, var_ano)) %>% 
  na.omit() %>% 
  mutate(data = seq(as.Date("1996-03-01"), by = "quarter", length.out = 93))

pib_tri %>% 
  ggplot(aes(x = data, y = var_tri)) +
...
- Data Science
    - [Tidyverse](https://www.rstudio.com/resources/videos/data-science-in-the-tidyverse/)
    ...
    - [Purr Tutorial](https://www.rstudio.com/resources/videos/happy-r-users-purrr/)
    - [List Columns](https://www.rstudio.com/resources/videos/using-list-cols-in-your-dataframe/)

- Tidyverse
    - [Tidyquant](https://www.rstudio.com/resources/videos/the-future-of-time-series-and-financial-analysis-in-the-tidyverse/)
    - [Lesser Known](https://www.rstudio.com/resources/videos/the-lesser-known-stars-of-the-tidyverse/)
...
- [Eric Zivot](http://faculty.washington.edu/ezivot/classes.htm)
